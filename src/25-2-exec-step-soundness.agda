...
soundness γ
  (exc αccounts
        (inj₁ (pr {ss = s} αcurrent αsender (state
...
soundness {Γ} γ ασ@(exc αccounts (inj₁ (pr {ss = s} αcurrent αsender αρ@(state αen
    (enf AMOUNT ; prg) rVM sVM Φ))) αpending)
  σ@(exc accounts (inj₁ (pr current sender ρ)) pending)
