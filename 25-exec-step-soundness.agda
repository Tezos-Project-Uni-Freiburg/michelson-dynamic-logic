...
soundness γ
  (αexc αccounts
        (inj₁ (αpr {s = s} αcurrent αsender (αstate
...
soundness {Γ} γ ασ@(αexc αccounts (inj₁ (αpr {s = s} αcurrent αsender αρ@(αstate αen
...
soundness {Γ} γ ασ@(αexc αccounts (inj₁ (αpr {s = s} αcurrent αsender αρ@(αstate αen
    (fct (D1 {result} x) ; prg) rVM sVM Φ))) αpending)
  σ@(exc accounts (just (pr current sender ρ)) pending)
  ( mβ , (refl , refl , refl , refl , mc , ms , mρ@(refl , refl , mE , refl , mρrest))
  , mp)
  with ρ-sound γ αρ ρ mρ
...
soundness γ ασ σ (mβ , ( refl , refl , refl , refl , mc , ms , mρ) , mp)
  ...
  = inj₂ ( _ , γ` , _ , 0∈ , (wkmodβ mβ) , (refl , refl , refl , refl
         , wkmodC {γ` = γ`} mc , (wkmodC {γ` = γ`} ms) , mρ`) , wkmodp mp)
soundness {Γ} γ ασ@(αexc αccounts (inj₁ (αpr {s = s} αcurrent αsender αρ@(αstate αen
    (IF-NONE {ty} thn els ; prg) (v∈ ∷ rVM) sVM Φ))) αpending)
  σ (mβ , ( refl , refl , refl , refl , mc , ms , mρ) , mp)
  | Γ` , γ` , _ , there (here px) , mρ` with ++-cancelʳ Γ Γ` [ ty ] (,-injectiveˡ px)
soundness γ ασ σ (mβ , ( refl , refl , refl , refl , mc , ms , mρ) , mp)
  | _ , γ` , _ , 1∈ , mρ` | refl
