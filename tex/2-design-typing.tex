\section{Michelson}\label{sec:Mtype}
...
\ACon{ADDnn}; and so on. If no  overloading is known for a combination
...
contracts) paired with the updated store.
\ConcreteContract

The $Mode$ argument abstracts over the semantics of types. Its type
has two components, one for the semantics and the other will be
...
Once a contract has terminated, the blockchain run-time updates the
