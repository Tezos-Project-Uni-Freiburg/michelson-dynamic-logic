...
\item In all other cases of a running program, the \ADT{ProgState} is executed
		using \ADT{prog-step}
	\item	The remaining cases are those where no contract is currently executed
		and \ADT{pending} is checked for other operations to be executed.
		...
		The \ADT{TRANSFERE-TOKENS} instruction only works for values of the
